...
public class VerifyAddProductInOpp_Sales extends CloudBaseTest
{

	@Test
	public void testAddNewProductToOpportunity() throws BiffException, IOException,
			InterruptedException, RowsExceededException, WriteException
	{
		...
		OpportunitiesDetail oppDetail = salesCloudOpportunitiesHome.clickOnNewButton().selectRecordType(recordType)
				.setOpportunityName(name).setAccountName(accountName).clickOnDefaultDateLink()
				.setOpportunityStage(oppStage).clickOnSaveButton().clickOnAddProduct()
				.verifyPageLoad().checkProductFromList(productName).clickOnSelectButton()
				.verifyPageLoad().enterQuantityForProductSelected(quantity).clickOnSave();

		
		/**
		 * Assertion
		 */
		assertTrue("Quantity saved incorrectly", oppDetail.getProductQuantity(productName).equals(quantity));
	}
